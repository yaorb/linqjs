...
module.exports = function (Enumerable) {
  ...
  const TooManyElementsException =
    Enumerable.exceptions.TooManyElementsException;
  const KeysForMultiElementsException =
    Enumerable.exceptions.KeysForMultiElementsException;
  ...
  assert.deepStrictEqual(
    Enumerable.range(2, 4).toArray(),
    Enumerable.between(2, 5).toArray(),
  );
  assert.deepStrictEqual(
    Enumerable.generate((i) => i * 2, 4).toArray(),
    [0, 2, 4, 6],
  );
  ...
  assert.deepStrictEqual(Enumerable.select([a, b, c], (v) => v + v).toArray(), [
    "aa",
    "bb",
    "cc",
  ]);
  assert.deepStrictEqual(Enumerable.select([oba, obb, obc], "key").toArray(), [
    a,
    b,
    c,
  ]);
  //where
  assert.deepStrictEqual(
    Enumerable.where([0, 1, 2, 3, 4, 5]).toArray(),
    [0, 1, 2, 3, 4, 5],
  );
  assert.deepStrictEqual(
    Enumerable.where([0, 1, 2, 3, 4, 5], (v) => v % 2 === 0).toArray(),
    [0, 2, 4],
  );
  assert.deepStrictEqual(
    Enumerable.where([oba, obb, obc, obu], "key").select("value").toArray(),
    [a, b, c],
  );
  assert.deepStrictEqual(
    Enumerable.where([oba, obb, obc, obu], Enumerable.predicates.not("key"))
      .select("value")
      .toArray(),
    [u],
  );
  ...
  assert.isStrictTrue(
    Enumerable.any(
      [oba, obb, obc, obu],
      Enumerable.predicates.not(Enumerable.predicates.selector("key")),
    ),
  );
  //all
  ...
  assert.strictEqual(
    Enumerable.average([1, 2, 3, 4], (v) => v * v),
    7.5,
  );
  ...
  //aggregate
  assert.strictEqual(
    Enumerable.aggregate([4, 3, 2, 1], 5, (seed, v) => seed + v),
    15,
  );
  assert.strictEqual(
  ...
  assert.strictEqual(
    Enumerable.select([5, 7], (v) => v + 2).maxOrDefault(1),
    9,
  );

  assert.strictEqual(Enumerable.max([5, 7, 3, 1, 9]), 9);
  assert.strictEqual(
    Enumerable.max([b, c, d, a], (v) => "-" + v),
    d,
  );
  assert.strictEqual(
    Enumerable.max(
      [b, c, d, a],
      (v) => "-" + v,
      (element, other) => (element > other ? -1 : element === other ? 0 : 1),
    ),
    a,
  );
  ...
  //concat
  assert.deepStrictEqual(Enumerable.concat([a, b], [d, e]).toArray(), [
    a,
    b,
    d,
    e,
  ...
  assert.strictEqual(
    Enumerable.count([1, 2, 3, 4, 5], (element) => element % 2 === 0),
    2,
  );
  ...
  assert.deepStrictEqual(Enumerable.defaultIfEmpty([a, b], a).toArray(), [
    a,
    b,
  ]);
  //distinct
  assert.deepStrictEqual(
    Enumerable.distinct([a, b, c, c, b, b, c, a]).toArray(),
    [a, b, c],
  );
  assert.deepStrictEqual(
    Enumerable.distinct(
      [1, 2, 3, 3, 2, 2, 3, 1],
      (element, value) => element % 2 === value % 2,
    ).toArray(),
    [1, 2],
  );
  //except
  assert.deepStrictEqual(
    Enumerable.except([a, b, c, d, e, c], [d, b]).toArray(),
    [a, c, e],
  );
  assert.deepStrictEqual(
    Enumerable.except(
      [1, 2, 3, 4, 5, 3, 6],
      [2, 4],
      (element, value) => element % 3 === value % 3,
    ).toArray(),
    [3],
  ); //in this equality comparer 3 is equal to 6, so number 6 was been ignored
  //union
  assert.deepStrictEqual(Enumerable.union([a, b, c], [b, c, d, e]).toArray(), [
    a,
    b,
    c,
    d,
  ...
  //itersect
  assert.deepStrictEqual(
    Enumerable.intersect([a, b, c], [b, c, d, e]).toArray(),
    [b, c],
  );
  assert.deepStrictEqual(
  ...
  assert.throws(
    () => Enumerable.elementAt([a, b, c, d, e], 10),
    OutOfRangeException,
  );
  assert.throws(
    () => Enumerable.elementAt([a, b, c, d, e], -1),
    OutOfRangeException,
  );
  //elementAtOrDefault
  ...
  //singleOrDefault
  assert.throws(
    () => Enumerable.singleOrDefault([a, b, c], f),
  ...
  //skip
  assert.deepStrictEqual(Enumerable.skip([a, b, c, d, e], 2).toArray(), [
    c,
    d,
    e,
  ]);
  //skipWhile
  assert.deepStrictEqual(
    Enumerable.skipWhile([1, 2, 3, 4, 5], (v) => v < 3).toArray(),
    [3, 4, 5],
  );
  //skipSame
  assert.deepStrictEqual(
    Enumerable.skipSame([1, 1, 1, 4, 5], (v) => v < 3).toArray(),
    [4, 5],
  );
  //skipProportion
  assert.deepStrictEqual(
    Enumerable.skipProportion([1, 1, 1, 4, 5], 0.5).toArray(),
    [1, 4, 5],
  );
  //take
  assert.deepStrictEqual(Enumerable.take([a, b, c, d, e], 3).toArray(), [
    a,
    b,
    c,
  ]);
  //takeWhile
  assert.deepStrictEqual(
    Enumerable.takeWhile([1, 2, 3, 4, 5], (v) => v <= 3).toArray(),
    [1, 2, 3],
  );
  //takeWhile
  assert.deepStrictEqual(
    Enumerable.takeSame([1, 1, 1, 4, 5], (v) => v <= 3).toArray(),
    [1, 1, 1],
  );
  //takeProportion
  assert.deepStrictEqual(
    Enumerable.takeProportion([1, 1, 1, 4, 5], 0.5).toArray(),
    [1, 1],
  );
  //proportion
  assert.strictEqual(
    Enumerable.proportion([1, 2, 3, 4, 5], (v) => v <= 3),
    0.6,
  );
  //zip
  assert.deepStrictEqual(
  ...
  //orderBy
  assert.deepStrictEqual(Enumerable.orderBy([d, a, c, b]).toArray(), [
    a,
    b,
    c,
    d,
  ...
  assert.deepStrictEqual(
    Enumerable.orderBy([d, a, c, b], (v) => v + v).toArray(),
    [a, b, c, d],
  );
  assert.deepStrictEqual(
    Enumerable.orderBy(
  ...
  assert.deepStrictEqual(
    Enumerable.orderBy([d, a, e, c, f, b], "", [c, b, d]).toArray(),
    [a, e, f, c, b, d],
  );
  //orderByDescending
  assert.deepStrictEqual(Enumerable.orderByDescending([d, a, c, b]).toArray(), [
    d,
    c,
    b,
    a,
  ...
  assert.deepStrictEqual(
    Enumerable.orderByDescending([d, a, c, b], (v) => v + v).toArray(),
    [d, c, b, a],
  );
  assert.deepStrictEqual(
  ...
  assert.deepStrictEqual(
    Enumerable.leftJoin(
      [1, 2, 3, 4],
      [2, 3, 4, 5],
      (outerElement, innerElement) => outerElement * (innerElement || 0),
    ).toArray(),
    [0, 4, 9, 16],
  );
  //rightJoin
  assert.deepStrictEqual(
    Enumerable.rightJoin(
      [1, 2, 3, 4],
      [2, 3, 4, 5],
      (outerElement, innerElement) => (outerElement || 0) * innerElement,
    ).toArray(),
    [4, 9, 16, 0],
  );
  //join
  ...
  assert.deepStrictEqual(
    Enumerable.join(
      ...
      [2, 3, 4, 5],
      (outerElement, innerElement) => outerElement.value * innerElement,
      (v) => v.key,
    ).toArray(),
    [6, 12, 20],
  );
  assert.deepStrictEqual(
    Enumerable.join(
      ...
      (outerElement, innerElement) =>
        outerElement.firstName + " " + innerElement.lastName,
      (outerElement) => outerElement.id,
      (innerElement) => innerElement.id,
    ).toArray(),
    ["e a", "f b"],
  );
  assert.deepStrictEqual(
    Enumerable.join(
      ...
      (outerElement, innerElement) =>
        outerElement.firstName + " " + innerElement.lastName,
      (outerElement) => outerElement.id,
      (innerElement) => innerElement.code,
      (element, other) => element === other - 100,
    ).toArray(),
    ["d b", "e c"],
  );
  //groupBy
  assert.deepStrictEqual(
    Enumerable.groupBy([3, 2, 1, 4, 3, 3, 1])
      .select((grouping) => grouping.toArray())
      .toArray(),
    [[3, 3, 3], [2], [1, 1], [4]],
  );
  assert.deepStrictEqual(
    Enumerable.groupBy([3, 2, 1, 4, 3, 3, 1], (v) => v % 3)
      .select((grouping) => grouping.toArray())
      .toArray(),
    [[3, 3, 3], [2], [1, 4, 1]],
  );
  ...
  assert.deepStrictEqual(
    Enumerable.groupBy(
      ...
      (v) => v * v,
      (key, grouping) => grouping.toArray(),
      (element, other) => element % 2 === other % 2,
    ).toArray(),
    [
      [9, 4, 9, 9],
      [1, 16, 1],
  ...
  //selectMany
  assert.deepStrictEqual(
    Enumerable.selectMany([[a, b, c], [d], [e, f]]).toArray(),
    [a, b, c, d, e, f],
  );
  assert.deepStrictEqual(
  ...
  assert.deepStrictEqual(
    Enumerable.groupJoin(
      [1, 2, 3, 4],
      [2, 4, 3, 4, 5],
      (outerElement, innerGrouping) => innerGrouping.toArray(),
    ).toArray(),
    [[], [2], [3], [4, 4]],
  );
  assert.deepStrictEqual(
  ...
  assert.deepStrictEqual(
    Enumerable.groupJoin(
      ...
      (outerElement, innerGrouping) =>
        outerElement.firstName +
        " " +
        innerGrouping.select((v) => v.lastName).join(" "),
      (outerElement) => outerElement.id,
      (innerElement) => innerElement.id,
    ).toArray(),
    ["d ", "e a", "f b !"],
  ...
  assert.deepStrictEqual(
    Enumerable.chunk([a, b, c, d, e, f], 3, 2)
      .skip(1)
      .select((c) => c.toArray())
      .toArray(),
    [[c, d, e], [f]],
  ...
  //split
  assert.deepStrictEqual(
    Enumerable.split([a, b, c, b, d, e], (element) => element === b)
      .select((chunk) => chunk.toArray())
      .toArray(),
    [[a], [c], [d, e]],
  );
  assert.deepStrictEqual(
    Enumerable.split([b, a, c, b, d, e], (element) => element === b)
      .select((chunk) => chunk.toArray())
      .toArray(),
    [[], [a, c], [d, e]],
  );
  assert.deepStrictEqual(
    Enumerable.split([a, b, c, b, d, e, b], (element) => element === b)
      .select((chunk) => chunk.toArray())
      .toArray(),
    [[a], [c], [d, e], []],
  );
  assert.deepStrictEqual(
    Enumerable.split([b, a, b, b, c, b, d, e, b], (element) => element === b)
      .select((chunk) => chunk.toArray())
      .toArray(),
    [[], [a], [], [c], [d, e], []],
  );
  assert.deepStrictEqual(
    Enumerable.split("babbcbdeb", /b/)
      .select((chunk) => chunk.join(""))
      .toArray(),
    ["", "a", "", "c", "de", ""],
  );
  assert.deepStrictEqual(
    Enumerable.nearSplit("babbcbdeb", /b/)
      .select((chunk) => chunk.join(""))
      .toArray(),
    ["", "a", "c", "de", ""],
  );
  //nearSplit
  assert.deepStrictEqual(
    Enumerable.nearSplit([a, b, c, b, b, d, e], (element) => element === b)
      .select((chunk) => chunk.toArray())
      .toArray(),
    [[a], [c], [d, e]],
  );
  assert.deepStrictEqual(
    Enumerable.nearSplit([b, b, a, c, b, d, e], (element) => element === b)
      .select((chunk) => chunk.toArray())
      .toArray(),
    [[], [a, c], [d, e]],
  );
  assert.deepStrictEqual(
    Enumerable.nearSplit([a, b, c, b, d, e, b, b], (element) => element === b)
      .select((chunk) => chunk.toArray())
      .toArray(),
    [[a], [c], [d, e], []],
  );
  assert.deepStrictEqual(
    Enumerable.nearSplit(
      [b, a, b, b, b, c, b, d, e, b],
      (element) => element === b,
    )
      .select((chunk) => chunk.toArray())
  ...
  //rightPad
  assert.deepStrictEqual(Enumerable.rightPad([a, b, c], 5, d).toArray(), [
    a,
    b,
    c,
    d,
  ...
  //leftPad
  assert.deepStrictEqual(Enumerable.leftPad([a, b, c], 5, d).toArray(), [
    d,
    d,
    a,
    b,
  ...
  //wipe
  assert.deepStrictEqual(
    Enumerable.wipe([a, b, c, a, b, c, a, b, c], (e) => e === a).toArray(),
    [b, c, b, c, b, c],
  );
  assert.deepStrictEqual(
    Enumerable.wipe([a, b, c, a, b, c, a, b, c], (e) => e === a, 2).toArray(),
    [b, c, b, c, a, b, c],
  );
  //nearBy
  assert.deepStrictEqual(
    Enumerable.nearBy([a, a, b, b, b, a, c, c])
      .select((g) => g.toArray())
      .toArray(),
    [[a, a], [b, b, b], [a], [c, c]],
  );
  ...
  //separate
  assert.deepStrictEqual(
    Enumerable.separate([[[a, b], c, d], [e, f], a]).toArray(),
    [a, b, c, d, e, f, a],
  );
  assert.deepStrictEqual(Enumerable.separate(tree).toArray(), nodes);
  assert.deepStrictEqual(
    Enumerable.separate(tree, (v) => v.children).toArray(),
    nodes,
  );
  //isSub
  ...
  //symmetric
  assert.deepStrictEqual(
    Enumerable.symmetric([1, 2, 3], [3, 4, 5]).toArray(),
    [1, 2, 4, 5],
  );
  //indices
  assert.deepStrictEqual(
    Enumerable.indices([a, b, c, d, e, f], [0, 2, 4]).toArray(),
    [a, c, e],
  );
  assert.deepStrictEqual(
    Enumerable.indices([a, b, c, d, e, f], [3, 2, 3, 5, 0]).toArray(),
    [d, c, d, f, a],
  );
  //permutation
  assert.deepStrictEqual(
    Enumerable.permutation([a, b, c], 2)
      .select((per) => per.toArray())
      .toArray(),
    [
      [a, b],
      [a, c],
  ...
  assert.deepStrictEqual(
    Enumerable.permutation([a, b, c, d, e], 3).count(),
    60,
  );
  assert.deepStrictEqual(
    Enumerable.permutation([a, b, c], 2, true)
      .select((per) => per.toArray())
      .toArray(),
    [
      [a, a],
  ...
  assert.deepStrictEqual(
    Enumerable.permutation([a, b, c, d, e], 3, true).count(),
    125,
  );
  assert.deepStrictEqual(
    Enumerable.permutation([a, b, c], 4, true).count(),
    81,
  );
  //combination
  assert.deepStrictEqual(
    Enumerable.combination([a, b, c], 2)
      .select((com) => com.toArray())
      .toArray(),
    [
      [a, b],
      [a, c],
  ...
  assert.deepStrictEqual(
    Enumerable.combination([a, b, c, d, e], 3).count(),
    10,
  );
  assert.deepStrictEqual(
    Enumerable.combination([a, b, c], 2, true)
      .select((com) => com.toArray())
      .toArray(),
    [
      [a, a],
  ...
  assert.deepStrictEqual(
    Enumerable.combination([a, b, c, d, e], 3, true).count(),
    35,
  );
  assert.deepStrictEqual(
    Enumerable.combination([a, b, c], 4, true).count(),
    15,
  );

  //IEnumerable methods
  //select
  assert.deepStrictEqual([a, b, c].asEnumerable().select().toArray(), [
    a,
    b,
    c,
  ]);
  ...
  //where
  assert.deepStrictEqual(
    [0, 1, 2, 3, 4, 5].asEnumerable().where().toArray(),
    [0, 1, 2, 3, 4, 5],
  );
  assert.deepStrictEqual(
    [0, 1, 2, 3, 4, 5]
  ...
  assert.strictEqual(
    [1, 2, 3, 4].asEnumerable().sum((v) => v * v),
    30,
  );
  ...
  assert.strictEqual(
    [1, 2, 3, 4].asEnumerable().average((v) => v * v),
    7.5,
  );
  assert.isStrictNaN([a, b, c].asEnumerable().average(), NaN);
  //aggregate
  assert.strictEqual(
    [4, 3, 2, 1].asEnumerable().aggregate(5, (seed, v) => seed + v),
    15,
  );
  assert.strictEqual(
    [4, 3, 2, 1].asEnumerable().aggregate(
      5,
      (seed, v) => seed + v,
      (r) => r * 2,
    ),
  ...
  assert.strictEqual(
    [b, c, d, a].asEnumerable().max((v) => "-" + v),
    d,
  );
  assert.strictEqual(
    [b, c, d, a].asEnumerable().max(
      (v) => "-" + v,
      (element, other) => (element > other ? -1 : element === other ? 0 : 1),
    ),
    a,
  ...
  assert.strictEqual(
    [b, c, d, a].asEnumerable().min((v) => "-" + v),
    a,
  );
  assert.strictEqual(
    [b, c, d, a].asEnumerable().min(
      (v) => "-" + v,
      (element, other) => (element > other ? -1 : element === other ? 0 : 1),
    ),
    d,
  ...
  //concat
  assert.deepStrictEqual([a, b].asEnumerable().concat([d, e]).toArray(), [
    a,
    b,
    d,
    e,
  ...
  assert.isStrictTrue(
    [1, 2, 3, 4, 5]
      .asEnumerable()
      .contains(6, (element, value) => element === value % 4),
  );
  //count
  assert.strictEqual([1, 2, 3, 4, 5].asEnumerable().count(), 5);
  assert.strictEqual(
    [1, 2, 3, 4, 5].asEnumerable().count((element) => element % 2 === 0),
    2,
  );
  //defaultIfEmpty
  assert.deepStrictEqual([].asEnumerable().defaultIfEmpty(a).toArray(), [a]);
  assert.deepStrictEqual([a, b].asEnumerable().defaultIfEmpty(a).toArray(), [
    a,
    b,
  ]);
  //distinct
  assert.deepStrictEqual(
    [a, b, c, c, b, b, c, a].asEnumerable().distinct().toArray(),
    [a, b, c],
  );
  assert.deepStrictEqual(
    [1, 2, 3, 3, 2, 2, 3, 1]
      .asEnumerable()
      .distinct((element, value) => element % 2 === value % 2)
      .toArray(),
    [1, 2],
  );
  //except
  assert.deepStrictEqual(
    [a, b, c, d, e, c].asEnumerable().except([d, b]).toArray(),
    [a, c, e],
  );
  assert.deepStrictEqual(
    [1, 2, 3, 4, 5, 3, 6]
      .asEnumerable()
      .except([2, 4], (element, value) => element % 3 === value % 3)
      .toArray(),
    [3],
  ); //in this equality comparer 3 is equal to 6, so number 6 was been ignored
  //union
  assert.deepStrictEqual(
    [a, b, c].asEnumerable().union([b, c, d, e]).toArray(),
    [a, b, c, d, e],
  );
  assert.deepStrictEqual(
    [1, 2, 3, 4]
      .asEnumerable()
      .union([5, 3, 6], (element, value) => element % 3 === value % 3)
      .toArray(),
    [1, 2, 3],
  );
  //itersect
  assert.deepStrictEqual(
    [a, b, c].asEnumerable().intersect([b, c, d, e]).toArray(),
    [b, c],
  );
  assert.deepStrictEqual(
    [1, 2, 3, 4]
      .asEnumerable()
      .intersect([5, 3, 6], (element, value) => element % 3 === value % 3)
      .toArray(),
    [2, 3],
  );
  //elementAt
  ...
  assert.throws(
    () => [a, b, c, d, e].asEnumerable().elementAt(10),
    OutOfRangeException,
  );
  assert.throws(
    () => [a, b, c, d, e].asEnumerable().elementAt(-1),
    OutOfRangeException,
  );
  //elementAtOrDefault
  assert.strictEqual(
    [a, b, c, d, e].asEnumerable().elementAtOrDefault(2, f),
    c,
  );
  ...
  assert.throws(
    () => [a, b, c].asEnumerable().single(),
    TooManyElementsException,
  );
  assert.throws(() => Enumerable.single([]), NoSuchElementsException);
  //singleOrDefault
  assert.throws(
    () => [a, b, c].asEnumerable().singleOrDefault(f),
    TooManyElementsException,
  );
  assert.strictEqual(Enumerable.singleOrDefault([], f), f);
  ...
  //reverse
  assert.deepStrictEqual([a, b, c].asEnumerable().reverse().toArray(), [
    c,
    b,
    a,
  ]);
  ...
  assert.isStrictTrue(
    [0, 1, 2]
      .asEnumerable()
      .sequenceEqual([3, 4, 5], (element, value) => element % 3 === value % 3),
  );
  //skip
  assert.deepStrictEqual([a, b, c, d, e].asEnumerable().skip(2).toArray(), [
    c,
    d,
    e,
  ]);
  ...
  //skipSame
  assert.deepStrictEqual(
    [1, 1, 1, 4, 5].asEnumerable().skipSame().toArray(),
    [4, 5],
  );
  //skipProportion
  assert.deepStrictEqual(
    [1, 1, 1, 4, 5].asEnumerable().skipProportion(0.5).toArray(),
    [1, 4, 5],
  );
  //take
  assert.deepStrictEqual([a, b, c, d, e].asEnumerable().take(3).toArray(), [
    a,
    b,
    c,
  ]);
  ...
  //takeSame
  assert.deepStrictEqual(
    [1, 1, 1, 4, 5].asEnumerable().takeSame().toArray(),
    [1, 1, 1],
  );
  //takeProportion
  assert.deepStrictEqual(
    [1, 1, 1, 4, 5].asEnumerable().takeProportion(0.5).toArray(),
    [1, 1],
  );
  //proportion
  assert.strictEqual(
    [1, 2, 3, 4, 5].asEnumerable().proportion((v) => v <= 3),
    0.6,
  );
  //zip
  assert.deepStrictEqual(
    [1, 2, 3]
      .asEnumerable()
      .zip([a, b, c], (element, other) => element + other)
      .toArray(),
    ["1a", "2b", "3c"],
  );
  //orderBy
  assert.deepStrictEqual([d, a, c, b].asEnumerable().orderBy().toArray(), [
    a,
    b,
    c,
    d,
  ...
  //orderByDescending
  assert.deepStrictEqual(
    [d, a, c, b].asEnumerable().orderByDescending().toArray(),
    [d, c, b, a],
  );
  assert.deepStrictEqual(
    [d, a, c, b]
  ...
  //selectMany
  assert.deepStrictEqual(
    [[a, b, c], [d], [e, f]].asEnumerable().selectMany().toArray(),
    [a, b, c, d, e, f],
  );
  assert.deepStrictEqual(
    [
  ...
  assert.deepStrictEqual(
    [1, 2, 3, 4]
      .asEnumerable()
      .groupJoin([2, 4, 3, 4, 5], (outerElement, innerGrouping) =>
        innerGrouping.toArray(),
      )
      .toArray(),
    [[], [2], [3], [4, 4]],
  );
  ...
  //rightPad
  assert.deepStrictEqual([a, b, c].asEnumerable().rightPad(5, d).toArray(), [
    a,
    b,
    c,
    d,
  ...
  //leftPad
  assert.deepStrictEqual([a, b, c].asEnumerable().leftPad(5, d).toArray(), [
    d,
    d,
    a,
    b,
  ...
  //separate
  assert.deepStrictEqual(
    [[[a, b], c, d], [e, f], a].asEnumerable().separate().toArray(),
    [a, b, c, d, e, f, a],
  );
  ...
  //symmetric
  assert.deepStrictEqual(
    [1, 2, 3].asEnumerable().symmetric([3, 4, 5]).toArray(),
    [1, 2, 4, 5],
  );
  //indices
  assert.deepStrictEqual(
    [a, b, c, d, e, f].asEnumerable().indices([0, 2, 4]).toArray(),
    [a, c, e],
  );
  assert.deepStrictEqual(
    [a, b, c, d, e, f].asEnumerable().indices([3, 2, 3, 5, 0]).toArray(),
    [d, c, d, f, a],
  );
  //permutation
  assert.deepStrictEqual(
  ...
  assert.deepStrictEqual(
    [a, b, c, d, e].asEnumerable().permutation(3).count(),
    60,
  );
  assert.deepStrictEqual(
    [a, b, c]
  ...
  assert.deepStrictEqual(
    [a, b, c, d, e].asEnumerable().permutation(3, true).count(),
    125,
  );
  assert.deepStrictEqual(
    [a, b, c].asEnumerable().permutation(4, true).count(),
    81,
  );
  //combination
  assert.deepStrictEqual(
  ...
  assert.deepStrictEqual(
    [a, b, c, d, e].asEnumerable().combination(3).count(),
    10,
  );
  assert.deepStrictEqual(
    [a, b, c]
  ...
  assert.deepStrictEqual(
    [a, b, c, d, e].asEnumerable().combination(3, true).count(),
    35,
  );
  assert.deepStrictEqual(
    [a, b, c].asEnumerable().combination(4, true).count(),
    15,
  );

  //for Syntax
  assert.deepStrictEqual(
    [...Enumerable.asEnumerable([1, 2, 3, 4, 5, 6]).where((v) => v % 2 === 0)],
    [2, 4, 6],
  );
  ...
  assert.deepStrictEqual(
    nodes
      ...
      .elementAt(0)
      .breadthSearch((node) => node.value === e),
    nodee,
  );
  assert.deepStrictEqual(
    nodes
  ...
  assert.deepStrictEqual(
    nodes
      ...
      .elementAt(0)
      .breadthSubTree((node) => node.value === e)
  ...
  assert.deepStrictEqual(
    nodes
      ...
      .elementAt(0)
      .depthSubTree((node) => node.value === c)
      .select((node) => node.value)
      .toArray(),
    [nodec],
  );
  ...
  (() => {
    ...
    assert.deepStrictEqual(theTree.pathTo(cTree).toArray(), [
      nodea,
      nodeb,
      nodec,
    ]);
  ...
  (() => {
    ...
    assert.deepStrictEqual(
      tree[0].asEnumerable((node) => node.children).toObject(),
      treeNoKey[0],
    );
    assert.deepStrictEqual(
      tree[0].asEnumerable((node) => node.children).toValue(),
      treeNoKey[0],
    );
  })();

  assert.deepStrictEqual(
    Enumerable.toPreOrder([a, b, c, d, e, f]).preOrder().toArray(),
    [a, b, c, d, e, f],
  );
  assert.deepStrictEqual(
    Enumerable.toInOrder([a, b, c, d, e, f]).inOrder().toArray(),
    [a, b, c, d, e, f],
  );
  assert.deepStrictEqual(
    Enumerable.toPostOrder([a, b, c, d, e, f]).postOrder().toArray(),
    [a, b, c, d, e, f],
  );
  assert.deepStrictEqual(
    [a, b, c, d, e, f].asEnumerable().toPreOrder().preOrder().toArray(),
    [a, b, c, d, e, f],
  );
  assert.deepStrictEqual(
    [a, b, c, d, e, f].asEnumerable().toInOrder().inOrder().toArray(),
    [a, b, c, d, e, f],
  );
  assert.deepStrictEqual(
    [a, b, c, d, e, f].asEnumerable().toPostOrder().postOrder().toArray(),
    [a, b, c, d, e, f],
  );
  ...
  (() => {
    ...
    //slice
    assert.deepStrictEqual(
      [1, 2, 3, 4, 5, 6].asEnumerable().slice(2, 5).toArray(),
      [3, 4, 5],
    );
    //copyWithin
    let array_copyWithin = ["alpha", "beta", "copy", "delta"].asEnumerable();
    assert.deepStrictEqual(array_copyWithin.copyWithin(1, 2, 3).toArray(), [
      "alpha",
      "copy",
      "copy",
      "delta",
    ]);
    assert.deepStrictEqual(array_copyWithin.toArray(), [
      "alpha",
      "copy",
      "copy",
      "delta",
    ...
    assert.deepStrictEqual(array_copyWithin.copyWithin(2, 0).toArray(), [
      "alpha",
      "bravo",
      "alpha",
      "bravo",
    ...
    //find
    assert.strictEqual(
      [12, 5, 8, 130, 44].asEnumerable().find((element) => element >= 15),
      130,
    );
    //findIndex
    assert.strictEqual(
      [12, 5, 8, 130, 44].asEnumerable().findIndex((element) => element >= 15),
      3,
    );
    //forEach
    //ignore
    ...
    //findLast
    assert.strictEqual(
      [2, 5, 9, 2].asEnumerable().findLast((element) => element % 2 === 1),
      9,
    );
    //map
    assert.deepStrictEqual(
    ...
    //reduce
    assert.strictEqual(
      [0, 1, 2, 3].asEnumerable().reduce((acc, val) => acc + val, 0),
      6,
    );
    let array_reduce1 = [
      [0, 1],
    ...
    //some
    assert.strictEqual(
      [2, 5, 8, 1, 4].asEnumerable().some((element) => element >= 10),
      false,
    );
    assert.strictEqual(
      [12, 5, 8, 1, 4].asEnumerable().some((element) => element >= 10),
      true,
    );
    //sort
    let array_sort = ["cherries", "apples", "bananas"].asEnumerable();
    assert.deepStrictEqual(array_sort.sort().toArray(), [
      "apples",
      "bananas",
      "cherries",
    ]);
    assert.deepStrictEqual(array_sort.toArray(), [
      "apples",
      "bananas",
      "cherries",
    ]);
  ...
  let testExtends = (lazy) => {
    ...
    assert.deepStrictEqual([a, b, c].select((v) => v + v).toArray(), [
      "aa",
      "bb",
      "cc",
    ]);
    //where
    assert.deepStrictEqual(
      [0, 1, 2, 3, 4, 5].where().toArray(),
      [0, 1, 2, 3, 4, 5],
    );
    assert.deepStrictEqual(
      [0, 1, 2, 3, 4, 5].where((v) => v % 2 === 0).toArray(),
      [0, 2, 4],
    );
    //any
    ...
    assert.strictEqual(
      [1, 2, 3, 4].average((v) => v * v),
      7.5,
    );
    assert.isStrictNaN([a, b, c].average(), NaN);
    //aggregate
    assert.strictEqual(
      [4, 3, 2, 1].aggregate(5, (seed, v) => seed + v),
      15,
    );
    assert.strictEqual(
      [4, 3, 2, 1].aggregate(
    ...
    assert.isStrictTrue(
      [1, 2, 3, 4, 5].contains(6, (element, value) => element === value % 4),
    );
    //count
    assert.strictEqual([1, 2, 3, 4, 5].count(), 5);
    assert.strictEqual(
      [1, 2, 3, 4, 5].count((element) => element % 2 === 0),
      2,
    );
    //defaultIfEmpty
    ...
    //distinct
    assert.deepStrictEqual([a, b, c, c, b, b, c, a].distinct().toArray(), [
      a,
      b,
      c,
    ]);
    ...
    //except
    assert.deepStrictEqual([a, b, c, d, e, c].except([d, b]).toArray(), [
      a,
      c,
      e,
    ]);
    ...
    //union
    assert.deepStrictEqual([a, b, c].union([b, c, d, e]).toArray(), [
      a,
      b,
      c,
      d,
    ...
    //skipWhile
    assert.deepStrictEqual(
      [1, 2, 3, 4, 5].skipWhile((v) => v < 3).toArray(),
      [3, 4, 5],
    );
    //skipSame
    ...
    //skipProportion
    assert.deepStrictEqual(
      [1, 1, 1, 4, 5].skipProportion(0.5).toArray(),
      [1, 4, 5],
    );
    //take
    ...
    //takeWhile
    assert.deepStrictEqual(
      [1, 2, 3, 4, 5].takeWhile((v) => v <= 3).toArray(),
      [1, 2, 3],
    );
    //takeSame
    ...
    //takeProportion
    assert.deepStrictEqual(
      [1, 1, 1, 4, 5].takeProportion(0.5).toArray(),
      [1, 1],
    );
    //proportion
    assert.strictEqual(
      [1, 2, 3, 4, 5].proportion((v) => v <= 3),
      0.6,
    );
    //zip
    assert.deepStrictEqual(
      [1, 2, 3].zip([a, b, c], (element, other) => element + other).toArray(),
      ["1a", "2b", "3c"],
    );
    //orderBy
    assert.deepStrictEqual([d, a, c, b].orderBy().toArray(), [a, b, c, d]);
    assert.deepStrictEqual([d, a, c, b].orderBy((v) => v + v).toArray(), [
      a,
      b,
      c,
      d,
    ...
    assert.deepStrictEqual(
      [d, a, c, b]
        .orderBy(
          (v) => v + v,
          (element, other) =>
            element > other ? -1 : element === other ? 0 : 1,
        )
        .toArray(),
      [d, c, b, a],
    );
    //orderByDescending
    assert.deepStrictEqual([d, a, c, b].orderByDescending().toArray(), [
      d,
      c,
      b,
      a,
    ...
    assert.deepStrictEqual(
      [d, a, c, b].orderByDescending((v) => v + v).toArray(),
      [d, c, b, a],
    );
    assert.deepStrictEqual(
      [d, a, c, b]
    ...
    //selectMany
    assert.deepStrictEqual([[a, b, c], [d], [e, f]].selectMany().toArray(), [
      a,
      b,
      c,
      d,
    ...
    //wipe
    assert.deepStrictEqual(
      [a, b, c, a, b, c, a, b, c].wipe((e) => e === a).toArray(),
      [b, c, b, c, b, c],
    );
    assert.deepStrictEqual(
      [a, b, c, a, b, c, a, b, c].wipe((e) => e === a, 2).toArray(),
      [b, c, b, c, a, b, c],
    );
    //nearBy
    assert.deepStrictEqual(
    ...
    //separate
    assert.deepStrictEqual([[[a, b], c, d], [e, f], a].separate().toArray(), [
      a,
      b,
      c,
      d,
    ...
    //symmetric
    assert.deepStrictEqual(
      [1, 2, 3].symmetric([3, 4, 5]).toArray(),
      [1, 2, 4, 5],
    );
    //indices
    assert.deepStrictEqual([a, b, c, d, e, f].indices([0, 2, 4]).toArray(), [
      a,
      c,
      e,
    ]);
    assert.deepStrictEqual(
      [a, b, c, d, e, f].indices([3, 2, 3, 5, 0]).toArray(),
      [d, c, d, f, a],
    );
    //permutation
    assert.deepStrictEqual(
    ...
    //filter
    assert.deepStrictEqual(
      [12, 5, 8, 130, 44].filter((value) => value >= 10).toArray(),
      [12, 130, 44],
    );
    //find
    assert.strictEqual(
      [12, 5, 8, 130, 44].find((element) => element >= 15),
      130,
    );
    //findIndex
    assert.strictEqual(
      [12, 5, 8, 130, 44].findIndex((element) => element >= 15),
      3,
    );
    //forEach
    //ignore
    ...
    //findLast
    assert.strictEqual(
      [2, 5, 9, 2].findLast((element) => element % 2 === 1),
      9,
    );
    //map
    assert.deepStrictEqual(
      [1, 5, 10, 15].map((x) => x * 2).toArray(),
      [2, 10, 20, 30],
    );
    assert.deepStrictEqual(
      [1, 4, 9].map((x) => Math.sqrt(x)).toArray(),
      [1, 2, 3],
    );
    //reduce
    assert.strictEqual(
      [0, 1, 2, 3].reduce((acc, val) => acc + val, 0),
      6,
    );
    let array_reduce1 = [
      [0, 1],
    ...
    let flatten_reduce = (arr) =>
      arr.reduce(
        (acc, val) =>
          acc.concat(Array.isArray(val) ? flatten_reduce(val) : val),
        [],
      );
    assert.deepStrictEqual(
      flatten_reduce(array_reduce1).toArray(),
      [0, 1, 2, 3, 4, 5],
    );
    assert.deepStrictEqual(
      flatten_reduce(array_reduce2).toArray(),
      [0, 1, 2, 3, 4, 5, 6],
    );
    //reduceRight
    assert.deepStrictEqual(
