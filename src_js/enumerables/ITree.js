...
class ITree extends GeneratorEnumerable {
  ...
  toValue(
    childrenSetter = defaultChildrenSetter,
    ...
    let children = this.select((sub) =>
      sub.toValue(childrenSetter, valueSetter),
    ).toArray();
    if (children.length) {
  ...
  lowestAncestor(...nodes) {
    ...
    if (result) {
      ...
      throw new NotAncestorOfException(
        this,
        nodes[
          isMatch
            .map((match, index) => ({ match, index }))
            .find(({ match }) => !match).index
        ],
      );
    }
  }
  ...
  degree(predicate = defaultPredicate) {
    ...
    return this.children.count((element, index) =>
      predicate(element.value, index),
    );
  }
  /**
   * 深度
  ...
  depth(predicate = defaultPredicate) {
    ...
    return (
      this.children
        .where((element, index) => predicate(element.value, index))
        .select((child) => child.depth(predicate))
        .maxOrDefault(0) + 1
    );
  }
  /**
  ...
  isFullBinary() {
    ...
    return (
      degree === 0 ||
      (degree === 2 && this.children.all((child) => child.isFullBinary()))
    );
  }
  ...
  static isSameNode(current, node, comparer = defaultEqualityComparer) {
    ...
    return (
      current === node ||
      (node instanceof ITree
        ? comparer(current.value, node.value)
        : comparer(current.value, node))
    );
  }
}
