...
const addExtends = (prototype, type, pascalOrPrefix, callback) => {
  ...
  if (
    prototypes.has(prototype) &&
    prototypes.get(prototype) !== pascalOrPrefix
  ) {
...
const removeExtends = (prototype, type, pascalOrPrefix) => {
  if (_extends.has(type)) {
    if (
      _extends.get(type).has(prototype) &&
      _extends.get(type).get(prototype) === pascalOrPrefix
    ) {
...
const extendObject = {
  ...
  filter(callback, thisArg) {
    if (
      (core.isArray(this) || core.isArguments(this)) &&
      core.a$filter &&
      !core.lazy
    ) {
  ...
  push(...values) {
    return Enumerable.push.apply(
      Enumerable,
      core.a$concat.call([this], values),
    );
  },
  shift() {
  ...
  unshift(...values) {
    return Enumerable.unshift.apply(
      Enumerable,
      core.a$concat.call([this], values),
    );
  },
  ...
  splice(start, count, ...values) {
    return Enumerable.splice.apply(
      Enumerable,
      core.a$concat.call([this, start, count], values),
    );
  },
  ...
  randomProbabilityOrDefault(
    ...
    return Enumerable.randomProbabilityOrDefault(
      this,
      defaultValue,
      probabilitySelector,
    );
  ...
  concat(...others) {
    if (core.isString(this) && core.s$concat && !core.lazy) {
      ...
      return Enumerable.concat.apply(
        Enumerable,
        core.a$concat.call([this], others),
      );
    }
  },
...
Enumerable.unextend = function (
  prototype,
  type,
  isPrototype = false,
  pascalOrPrefix = false,
) {
  if (typeof prototype !== "object" || core.getType(type) !== core.types.String)
    return prototype;
  if (!isPrototype || removeExtends(prototype, type, pascalOrPrefix)) {
    core.undefineProperties(
      prototype,
      [...Object.keys(extendObject), "toDictionary", "toLookup"],
      pascalOrPrefix,
    );
...
Enumerable.extend = function (
  ...
  if (
    !isPrototype ||
    addExtends(prototype, type, pascalOrPrefix, (pascalOrPrefix) =>
      this.unextend(prototype, type, isPrototype, pascalOrPrefix),
    )
  ) {
