...
const CONFLICT_SET_CONFIG =
  'Can not set this config after call the noConflict method. If you want, you can use Enumerable.noConflict method with one parameter which value is "true" to set Enumerable of global back to this module';
...
Enumerable.restore = function (saved) {
  ...
  this.config.extends.array =
    (saved.config && saved.config.extends && saved.config.extends.array) ||
    false;
  this.config.extends.string =
    (saved.config && saved.config.extends && saved.config.extends.string) ||
    false;
  this.config.extends.object =
    (saved.config && saved.config.extends && saved.config.extends.object) ||
    false;
