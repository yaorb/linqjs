const getFunctionNameReg = /^(function\*?|class)\s+([^({\s]*)\s*[({].*/gi;
const getObjectTypeNameReg = /^\[\w+\s(.+)]$/gi;

const getter = (properties, property) => {
  return () => properties[property];
};

const getFunctionName = (fun): string =>
  fun.name || (getFunctionNameReg.exec(fun) || [])[2] || "";
...
export const core = {
  ...
  getType(value: any): string {
    ...
    if (typeName === "undefined") {
      ...
      let type: string = value[Symbol.toStringTag];
      if (!type) {
  ...
  isEnumerable(value: any) {
    return core.getType(value).endsWith(ENUMERABLE);
  },
  isProto(value: any) {
    const type = core.getType(value);
    ...
    if (typeof property !== "symbol" && prototype.hasOwnProperty(property)) {
      ...
      if (core.isDev())
        console.warn(
          property +
            " already in " +
            core.getType(prototype) +
            ", set original function to " +
            newProperty,
        );
  ...
  defineProperty(
    ...
    if (property === Symbol.iterator) {
      let name = (
        getFunctionName(value) || getFunctionName(prototype[Symbol.iterator])
      ).replace(/\s*Iterator$/gi, " Iterator");
      if (name) {
    ...
    for (let property in properties) {
      if (properties.hasOwnProperty(property)) {
        core.defineProperty(
          prototype,
          pascalOrPrefix === true
            ? core.asPascal(property)
            : pascalOrPrefix
            ? pascalOrPrefix + property
            : property,
          getter(properties, property),
          true,
          false,
        );
      }
  ...
  asEnumerable(object: any, childrenSelector?: any, valueSelector?: any) {
    if (core.isUndefined(childrenSelector) && core.isEnumerable(object)) {
      return object;
    }

    let c;
    if (core.isUndefined(childrenSelector)) {
      if (core.isIterator(object)) {
  ...
  repeat(element: any, count: number) {
    const result: any[] = [];
  ...
  swap(array: any[], index: number, other: any) {
  ...
  upping(array, index, direction, comparer) {
    ...
    if (length >= 1) {
      if (direction < 0) {
        ...
        for (
          let i = index, t = length - ((length - i) >> 1);
          i < length - 1;
          i = t, t = length - ((length - t) >> 1)
        ) {
          // console.log(i, t);
