...
const randomNodeProbability = function (
  enumerable,
...
class IEnumerable<T> extends Array<T> {
  constructor(source) {
    super();
    const typeName = core.getType(source);
    const type =
      source instanceof IEnumerable
        ? enumerable
        : typeName === core.types.String
        ? string
        : typeName === core.types.Array ||
          typeName.endsWith(core.types.Iterator)
        ? array
        : object;
    ...
    core.defineProperties(this, {
      ...
      toString() {
        return type === string
          ...
          ? "[" + this.join(",") + "]"
          : type === enumerable
          ? source.toString.call(this)
          : "[" + this.join(",") + "]";
      },
      toProto() {
        return this.getProtoType() === string
          ? this.join("")
          : this.getProtoType() === array
          ? this.toArray()
          : this.toObject();
      },
    });
        ...
        getOwnPropertyDescriptor(target, prop: PropertyKey) {
          if (
            ...
            return {
              enumerable: true,
              configurable: true,
              get: () => target.elementAtOrDefault(prop),
            };
          }

          if (
            typeof prop === "string" &&
            (prop === "length" || prop === "size")
          ) {
            const desc = Object.getOwnPropertyDescriptor(target, "length");
            desc && (desc.value = target.count());
            return desc;
          }
  ...
  concat(...others) {
    ...
    return new (Function.prototype.bind.apply(
      ConcatEnumerable,
      core.a$concat.call([null], [this], others),
    ))();
  }
  ...
  sequenceEqual(other, comparer = defaultEqualityComparer) {
    ...
    while (
      !(
        (sourceElement = sourceIterator.next()).done &
        (otherElement = otherIterator.next()).done
      )
    ) {
  ...
  findIndex(predicate, thisArg) {
    ...
    let callback = (element, index) =>
      predicate.call(thisArg, element, index, this);
    for (let element of this) {
  ...
  findLast(callback, thisArg) {
    return this.lastOrDefault(undefined, (element, index) =>
  ...
  every(callback, thisArg) {
    return this.all((element, index) =>
      callback.call(thisArg, element, index, this),
    );
  }
  find(callback, thisArg) {
    return this.firstOrDefault(undefined, (element, index) =>
      callback.call(thisArg, element, index, this),
    );
  }
  ...
  map(callback, thisArg) {
    return this.select((element, index) =>
      callback.call(thisArg, element, index, this),
    );
  }
  filter(callback, thisArg) {
    return this.where((element, index) =>
      callback.call(thisArg, element, index, this),
    );
  }
  pop() {
  ...
  reduce(callback, initialValue) {
    return this.aggregate(initialValue, (seed, element, index) =>
      callback(seed, element, index, this),
    );
  }
  reduceRight(callback, initialValue) {
    return this.reverse().aggregate(initialValue, (seed, element, index) =>
      callback(seed, element, index, this),
    );
  }
  some(callback, thisArg) {
    return this.any((element, index) =>
      callback.call(thisArg, element, index, this),
    );
  }
  splice(start, count, ...values) {
    ...
    return new (Function.prototype.bind.apply(
      SpliceEnumerable,
      core.a$concat.call([null], [this, start, count], values),
    ))();
  }
  ...
  forEach(action = defaultAction, thisArg = undefined) {
    let index = 0;
    let callback = (element, index) =>
      action.call(thisArg, element, index, this);
    for (let element of this) {
  ...
  nearBy(
    ...
    return new NearGroupedEnumerable(
      this,
      keySelector,
      elementSelector,
      resultSelector,
